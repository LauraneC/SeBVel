...
cube_name = 'c_x01225_y03675.nc'

cube_IGE_path = "/media/charriel/Elements/Donnees_IGE_Alpes/cubes_Sentinel_2_2022_2023/Mont_Blanc"

cube_list = glob.glob(cube_IGE_path + "/*.nc")

for cube_name_IGE in tqdm(cube_list):
    print(cube_name_IGE)
    cube_grid = gpd.read_file('/media/charriel/Elements/Donnees_IGE_Alpes/cubes_S2_avant2022/cube_grid.shp')
    cube_samp = cube_grid[cube_grid['NAME'] == f'{cube_name_IGE.split("/")[-1].split(".nc")[0]}_post0050_yr*']
    ...
    cube_name = {
        "raw": 'http://its-live-data.s3.amazonaws.com/datacubes/v2-updated-october2024/N40E000/ITS_LIVE_vel_EPSG32632_G0120_X350000_Y5050000.zarr'}

    # mask_file = '/media/charriel/Elements/Donnees_IGE_Alpes/metadata/stable_areas.gpkg'  # Path to mask file (.shp file) to mask some of the data on cube
    mask_file = None
    ...
    path_save = f'/media/charriel/Elements/Donnees_IGE_Alpes/Saisonality/'  # Path where to store the results
    result_fn = "c_x01225_y03675_x_0_y_1"  # Name of the netCDF file to be created (if save is True)

    proj = f"EPSG:{cube_grid.crs.to_epsg()}"  # EPSG system of the given coordinates
    ...
    load_kwargs = {
        ...
        "pick_date": ['2015-06-01', '2022-12-31'],  # Select dates ([min, max] or None to select all)
